		...
		function() {
			...
			if (hrefId == "track") {
				layer_show_refresh_click('详细进货记录',
						'/cr-admin/stock/toIncomeItemStockList.do?stockId='
...
function initStatisticsData() {
	...
	var startTime = document.getElementById("startTime").value;
	var endTime = document.getElementById("endTime").value;
	var url = "/cr-admin/stock/getComingStockList.do";
	$.ajax({
		url : url,
		type : "post",
		data : {
			...
			.DataTable(
					{
						...
						"columnDefs" : [
								...
								},
								{
									...
									targets : -1,
									render : function(data, type, row, meta) {
										var tdHtml = "<td class=\"td-status\"><a title=\"详细进货记录\" href=\"javascript:;\" class=\"track\"><span class=\"label label-success radius\">详细进货记录</span></a>"
												+ "</td>";
										return tdHtml;
									}
								},
						...
						"initComplete" : function(settings, json) {// table初始化后触发
							...
							$("table tr").find("td:eq(7),th:eq(7)").hide();// 设置第一列隐藏
						},
						"drawCallback" : function(settings) {
							$("table tr").find("td:eq(1)").hide();// 设置第一列隐藏
							$("table tr").find("td:eq(7),th:eq(7)").hide();
...
function expExcel() {
	...
	var startTime = document.getElementById("startTime").value;
	var endTime = document.getElementById("endTime").value;
	var url = "/cr-admin/stock/expStockComeList.do";
	$.ajax({
		url : url,
		type : "post",
		data : {
