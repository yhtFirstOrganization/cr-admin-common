...
$("tbody").on(
		...
		function() {
			...
			var aobject = window.parent.parent.document.getElementById("repairListId");
			var _hrefVal = $(aobject).attr("_href");
			var ahtml = $(aobject).html();
			// 修改值
			$(aobject).attr("_href", _hrefVal + "?carid=" + carId);
			$(aobject).html("维修记录");
			window.parent.parent.document.getElementById("repairListId").click();
			// 恢复原值
...
function initStatisticsData() {
	...
	var url = "/cr-admin/carUseStock/getCarUseStockList.do";
	$.ajax({
		url : url,
		type : "post",
		data : {
			...
			.DataTable(
					{
						...
						"columnDefs" : [
								...
								},
								{
									...
									targets : -1,
									render : function(data, type, row, meta) {
										var tdHtml = "<td class=\"td-status\"><a title=\"详细维修记录\" href=\"javascript:;\" class=\"track\"><span class=\"label label-success radius\">详细维修记录</span></a>"
												+ "</td>";
										return tdHtml;
									}
								},
