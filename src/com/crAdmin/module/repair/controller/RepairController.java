...
public class RepairController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("/toRepairItem.do")
    // @LogAnnotation(option = "跳转到修改维修记录和项目信息页面", optionType =
    // OptionType.pageIn)
    public ModelAndView toRepairItem(Integer repairId) {
        ModelAndView mv = new ModelAndView(
                "repairAndItem/repairItem.ftl");
        ...
        prepair.setIsdelete("0");
        Repair queryRepair = repairService.queryRepairInfo(prepair);
        Date repairTime = queryRepair.getRepairTime();
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        queryRepair.setRepairTimeStr(format.format(repairTime));
